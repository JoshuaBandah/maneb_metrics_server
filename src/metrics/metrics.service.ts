...
import { K6MonitorService } from '../k6-monitor/k6-monitor.service';
...
export class MetricsService {
  ...
  private calculateDashboardMetrics() {
    ...
    const totalRequests = Math.max(
      0,
      currentTotal - this.lastRequestCounts.total,
    );

    const failedRequests = Math.max(
      0,
      currentFailed - this.lastRequestCounts.failed,
    );
    ...
    if (
      this.lastCpu.total > 0 &&
      cpuTotal >= this.lastCpu.total
    ) {
      ...
      if (timeDelta > 0) {
        ...
        cpuUsage = Math.min(
          100,
          (cpuDelta / (timeDelta * cpuCount)) * 100,
        );
      }
    }
    ...
    return {
      ...
      clientSideRequest: this.clientSideData,
    };
  }
  ...
  getDebugInfo() {
    return {
      ...
      clientSideData: this.clientSideData,
    };
  }
}